...
export default function Home() {
  ...
  // Poll status and logs while scraper is running so UI updates when complete
  useEffect(() => {
  ...
  // When status.isRunning becomes false (from poll), effect cleanup clears interval; button updates
  ...
  const startScraper = async () => {
    try {
      ...
      addLog('error', error.response?.data?.error || 'Failed to start scraper');
  ...
  // Remove pollScraperStatus function - no longer needed
  ...
  const logTypeClass = (type) => {
    ...
    return styles[key] ?? styles.logInfo;
  };
    ...
    <div className={styles.container}>
      ...
      <div className={styles.card}>
        ...
        <div className={styles.controls}>
          ...
          <div className={styles.inputGroup}>
            ...
            <input
              type="number"
              value={config.jobAge}
              onChange={(e) => setConfig({ ...config, jobAge: parseInt(e.target.value) })}
              min="1"
              max="30"
              disabled={status.isRunning}
            />
        ...
        <div className={styles.sessionBox}>
          ...
          <div className={styles.sessionRow}>
            Normal: {sessionStatus.normalSession.exists ? '✅ Active' : '❌ Not found'} | 
            MNC: {sessionStatus.mncSession.exists ? '✅ Active' : '❌ Not found'}
          </div>
      ...
      <div className={`${styles.card} ${styles.statusSection}`}>
        ...
        <div className={styles.statusGrid}>
          ...
          <div className={styles.statCard}>
            <h3>{status.dailyLimit}/{status.maxLimit}</h3>
            <p>Daily Limit</p>
          </div>
        </div>
        ...
        <div className={styles.logContainer}>
          {logs.length === 0 ? (
            ...
            <>
              {logs.map((log, index) => (
                <div key={index} className={`${styles.logEntry} ${logTypeClass(log.type)}`}>
                ...
                <tbody>
                  {jobHistory.length === 0 ? (
                    ...
                    jobHistory.map((job, index) => {
                      const isApplied = job.appliedAt !== 'N/A' && !job.reason.includes('failed');
                      const isSkipped = job.reason !== 'N/A' && job.appliedAt === 'N/A';
                      const badgeClass = isApplied ? styles.badgeApplied : isSkipped ? styles.badgeSkipped : styles.badgeDefault;
                      return (
